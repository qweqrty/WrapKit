...
public class KeyValueFieldViewOutputSwiftUIAdapter: ObservableObject, KeyValueFieldViewOutput {
    ...
    enum OutputReplayEvent {
        ...
        case displayKeyTitle(DisplayKeyTitleState)
    ...
    func replayOutputEvent(
        ...
        case .displayKeyTitle(let state):
            outputReplayStateDelivery.send(state)
    ...
    @Published public var displayKeyTitleState: DisplayKeyTitleState? = nil {
        willSet {
            guard let state = newValue else { return }
            beginOutputReplayDelivery(.displayKeyTitle(state))
        }
        didSet {
            guard displayKeyTitleState != nil else { return }
            finishOutputReplayDelivery()
        }
    }
    ...
    public func display(keyTitle: TextOutputPresentableModel?) {
        requireMainOutputReplayQueue()
        let keyTitle = outputReplayClosureStore.sanitize(
            keyTitle,
            path: "first"
        )
        nextOutputSequence &+= 1
