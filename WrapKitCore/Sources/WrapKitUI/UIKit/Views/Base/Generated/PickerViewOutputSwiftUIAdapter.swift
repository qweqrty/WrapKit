...
public class PickerViewOutputSwiftUIAdapter: ObservableObject, PickerViewOutput {
    ...
    private func synchronizeOutputClosureProperties(_ update: () -> Void) {
        ...
        @Published public var componentsCount: (() -> Int?)? = nil {
            willSet {
                ...
                if !isSynchronizingOutputClosureProperties {
                    outputReplayClosureStore.invalidate(prefix: "componentsCount")
                }
            }
        }
        @Published public var rowsCount: (() -> Int)? = nil {
            willSet {
                dispatchPrecondition(condition: .onQueue(.main))
                if !isSynchronizingOutputClosureProperties {
                    outputReplayClosureStore.invalidate(prefix: "rowsCount")
                }
            }
        }
        @Published public var titleForRowAt: ((Int) -> String?)? = nil {
            willSet {
                dispatchPrecondition(condition: .onQueue(.main))
                if !isSynchronizingOutputClosureProperties {
                    outputReplayClosureStore.invalidate(prefix: "titleForRowAt")
                }
            }
        }
        @Published public var didSelectAt: ((Int) -> Void)? = nil {
            willSet {
                dispatchPrecondition(condition: .onQueue(.main))
                if !isSynchronizingOutputClosureProperties {
                    outputReplayClosureStore.invalidate(prefix: "didSelectAt")
                }
            }
        }
    ...
    enum OutputReplayEvent {
        ...
        case displaySelectedRow(DisplaySelectedRowState)
    }
    ...
    func replayOutputEvent(
        ...
        case .displaySelectedRow(let state):
            outputReplayStateDelivery.send(state)
        }
    ...
    @Published public var displaySelectedRowState: DisplaySelectedRowState? = nil {
        willSet {
            guard let state = newValue else { return }
            beginOutputReplayDelivery(.displaySelectedRow(state))
        }
        didSet {
            guard displaySelectedRowState != nil else { return }
            finishOutputReplayDelivery()
        }
    }
    ...
    public func display(selectedRow: PickerViewPresentableModel.SelectedRow?) {
        ...
        let sanitizedSelectedRow = outputReplayClosureStore.sanitize(
            selectedRow,
            path: "selectedRow"
        )!
        nextOutputSequence &+= 1
        displaySelectedRowState = .init(
            outputSequence: nextOutputSequence,
            selectedRow: sanitizedSelectedRow
        )
    }
}
