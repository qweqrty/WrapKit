...
public class ProgressBarOutputSwiftUIAdapter: ObservableObject, ProgressBarOutput {
    ...
    enum OutputReplayEvent {
        ...
        case displayProgress(DisplayProgressState)
    ...
    func replayOutputEvent(
        ...
        case .displayProgress(let state):
            outputReplayStateDelivery.send(state)
    ...
    @Published public var displayProgressState: DisplayProgressState? = nil {
        willSet {
            guard let state = newValue else { return }
            beginOutputReplayDelivery(.displayProgress(state))
        }
        didSet {
            guard displayProgressState != nil else { return }
            finishOutputReplayDelivery()
        }
    }
